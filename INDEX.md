...
| `docs/state/CHANGELOG.md` | project root | Keep-a-Changelog style release history. | yes |
| `docs/state/TODO.md` | project root | P0/P1/P2 tasks with requirement IDs (`AXI-AP-<AREA>-<NN>`) and test cases for P0/P1. | yes |
| `docs/state/MILESTONE.md` | project root | Evidence-backed milestone and current status. | yes |
| `docs/governance/SECURITY.md` | project root | Security reporting policy. | yes |
| `docs/state/BUG_FIXES.md` | project root | Historical bug-fix log. | reference only |
| `docs/state/UPGRADE_v1.1.0.md` | project root | v1.1.0 SubAgent upgrade narrative (source language: Chinese). | yes |
| `docs/state/UPGRADE_v1.1.0.zh-CN.md` | project root | Identity mirror of `UPGRADE_v1.1.0.md`. | mirror |
| `docs/state/升级方案.md` | project root | Long-form v1.x upgrade plan in Chinese. | yes (historical) |
| `docs/state/upgrade-plan.zh-CN.md` | project root | Identity mirror of `升级方案.md`. | mirror |
| `docs/state/参考.md` | project root | Reference notes for upstream products and inspiration (third-party / reference governance). | reference only |

## `docs/` documents

| Path | Owner | Purpose | Source of truth? |
|------|-------|---------|------------------|
| `docs/state/PRD.md` | docs-agent | Product requirements: users, acceptance criteria, non-goals. | yes |
| `docs/state/TDD.md` | docs-agent | Test / technical design: architecture assumptions, test strategy, verification commands, risk cases. | yes |
...
3. `docs/state/UPGRADE_v1.1.0.zh-CN.md` (SubAgent mode overview).
4. `infra/axi-agent-mcp/README.zh-CN.md` (MCP swarm surface).
5. `INDEX.md` (this file).
6. `docs/state/PRD.md` and `docs/state/TDD.md` (requirements and design).
7. `docs/state/TODO.md` and `docs/state/MILESTONE.md` (current state).
8. `docs/state/CHANGELOG.md` (release history).
...
- Reference / third-party governance docs (`docs/state/参考.md`, `docs/audit/`, etc.) must not be promoted to source-of-truth status without an explicit ADR.
