...
function printHeader() {
  ...
  console.log(`${c}${BOLD}║  Axi Agent Transport — ${label.padEnd(19)}║${RESET}`);
  console.log(`${c}${BOLD}╚══════════════════════════════════════════╝${RESET}`);
}
...
function spawnClaude() {
  ...
  // On Windows, .cmd/.bat files must run via cmd.exe /c
  const isWindows = process.platform === 'win32';
  const cmd = isWindows && config.claudeCmd.endsWith('.cmd')
    ? 'cmd.exe'
    : config.claudeCmd;
  const cmdArgs = isWindows && config.claudeCmd.endsWith('.cmd')
    ? ['/c', config.claudeCmd, ...config.claudeArgs]
    : config.claudeArgs;

  claudeProcess = spawn(cmd, cmdArgs, {
 ...
 *
 * Claude stream-json output (verified with direct test):
 *   {"type":"system","subtype":"hook_started",...}  — session hook start (ignore)
 *   {"type":"system","subtype":"hook_response",...} — hook output (ignore)
 *   {"type":"system","subtype":"init",...}          — session init (ignore)
 *   {"type":"assistant","message":{...}}            — complete assistant message
 *   {"type":"result","subtype":"success|error",...} — final result + routing info
 */
function handleClaudeEvent(event) {
  switch (event.type) {
    // ── Assistant message: extract text and accumulate into streamBuffer ───────
    case 'assistant': {
      ...
      for (const block of msg.content) {
        if (block.type === 'text') {
          ...
          if (thought) streamBuffer += thought;
        }
        // Accumulate everything into streamBuffer (capped below)
    ...
    // ── Final result: response complete, parse for directives ────────────────
    case 'result': {
    ...
    // ── Legacy streaming events (may appear in some CLI versions) ────────────
    ...
    // ── Structural events that don't need processing ────────────────────────
    case 'message_start':
    ...
    default:
      // Unknown event types (e.g. system) — ignore
      break;
  }
}
