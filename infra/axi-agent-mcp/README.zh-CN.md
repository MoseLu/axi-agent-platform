...
Axi Agent 的 MCP 服务所有者（service owner）。根据任务内容自动选择模型并调用你的 API，与 `.cursor/rules/multi-model-swarm.mdc` 规则保持一致。**MiniMax-M2.5** 已并入蜂群（作为创意专家），可为其配置独立 API（`SWARM_MINIMAX_*`），与统一 API 并行使用。
